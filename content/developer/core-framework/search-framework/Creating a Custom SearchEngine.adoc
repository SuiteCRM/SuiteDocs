...
aliases:
    - /developer/searchframework/creating-a-custom-searchengine/
---
...
class CitySearch extends SearchEngine
{
    public function search(SearchQuery $query)
    {
        ...
        $city = $db->quote($query->getSearchString());

        $sql = "SELECT id FROM accounts WHERE billing_address_city = '$city' OR shipping_address_city = '$city' LIMIT %d OFFSET %d";

        // Format the query with numeric values (size and offset)
        $sql = sprintf(
            $sql,
            $query->getSize(), // LIMIT
