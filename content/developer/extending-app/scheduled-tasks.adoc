...
this is sending an email in a logic hook (see the link:../../core-framework/logic-hooks[Logic Hooks] chapter
...
Next we create the BeanEmailJob class. This is placed into the `custom/Extension/modules/Schedulers/Ext/ScheduledTasks/` directory
with the same name as the class. So in our example we will have: +
`custom/Extension/modules/Schedulers/Ext/ScheduledTasks/BeanEmailJob.php`
...
See the section on link:../../core-framework/logic-hooks/[logic hooks]
