...
aliases:
    - /developer/performance-tweaks/
---

= Performance Tweaks

Quick configuration wins that improve performance on any SuiteCRM instance. For architecture-level optimisation targeting large datasets or high-concurrency environments, see the link:../performance-tuning/[Performance Tuning] page.
...
of some PHP accelerators which will further increase performance.
https://www.php.net/manual/en/book.opcache.php[OPcache^] is built into
PHP 7+ and should be enabled on all production servers. Windows users
should also check out
https://www.php.net/manual/en/book.wincache.php[WinCache^]. For
recommended OPcache configuration values, see the
link:../performance-tuning/[Performance Tuning] page.
...
the link:../../core-framework/vardefs/[Vardef] chapter for adding
indexes.
...
chapter on the link:../../extending-app/config/[Config] files for more
information.
