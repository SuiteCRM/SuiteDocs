...
Add the labels via a YAML file in your extension (e.g. `extensions/defaultExt/modules/Contacts/language/duplicate-email-validation.yaml`).
...
To add module-level labels (`mod_strings`) instead, use the `module` key with the module name:
...
Alternatively, add to the global app strings file (e.g. `public/legacy/include/language/en_us.lang.php`) or via the legacy extension directory (e.g. `public/legacy/custom/Extension/application/Ext/Language/en_us.duplicate_email.php`):

[source,php]
----
<?php

$app_strings['LBL_DUPLICATE_EMAIL_TITLE'] = 'Possible Duplicate Contacts';
$app_strings['LBL_DUPLICATE_EMAIL_MESSAGE'] = 'The following contacts already have this email address. Do you still want to proceed?';
$app_strings['LBL_DUPLICATE_EMAIL_FOUND'] = 'Duplicate email address found';
----
...
* link:../../../extensions/frontend/containers/confirmation-modal/confirmation-modal[Confirmation Modal] — text confirmation modal reference and configuration properties
