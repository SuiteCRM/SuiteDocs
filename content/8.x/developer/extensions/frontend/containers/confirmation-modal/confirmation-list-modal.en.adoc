...
For an example of how this modal is used, see the duplicate email check in the link:../../../../../metadata/record-view-metadata/record-async-validators[Record-Level Async Validators] page (Section 7).
...
All links in the table open the record from the modal's module in a new tab — not the related record. For example, an `account_name` column on a Contacts list will open the Contact record, not the Account. Because of this, it is best practice to set `link: false` on relate fields to avoid confusing navigation.

By default, `relate` type fields have their links automatically disabled. Other field types (such as `name`) keep their default link behaviour. To explicitly control link behaviour per column:

* `['name' => 'name']` — keeps the default link behaviour (opens the record in a new tab)
* `['name' => 'email1', 'link' => false]` — disables the link
* `['name' => 'account_name', 'link' => true]` — forces the link on (not recommended for relate fields)
