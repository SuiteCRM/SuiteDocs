...
weight: 20
aliases:
    - /developer/api/api-4_1/
---

{{% notice warning %}}
This page documents the legacy v4.1 API. It is provided for reference only. New integrations should use the link:../../v8/[V8 API], which is based on the JSON API specification and uses OAuth2 for authentication.
{{% /notice %}}
...
{{% notice note %}}
The v4.1 API requires the password to be passed as an MD5 hash. This is a protocol requirement of the API itself and cannot be changed. For stronger authentication, use the link:../../v8/[V8 API] with OAuth2.
{{% /notice %}}
...
$results = $client->get_entry_list(
        ...
        "accounts.billing_address_state = 'Ohio'",
        //Order by - unused
        '',
...
`method`::
  ...
  link:../endpoints[list of API v4.1 methods].
`input_type`::
...
$entryArgs = array(
 ...
 //Filter query - Added to the SQL where clause,
	'query' => "accounts.billing_address_state = 'Ohio'",
 //Order by - unused
	'order_by' => '',
 //Start with the first record
	'offset' => 0,
