...
Read more about this scheduler and how to configure it in link:../automatic-email-importing/[Automatic Email Importing].
...
If inbound email accounts are not synchronising correctly, use the *Sync Inbound Email Accounts* tool to reconnect accounts via IMAP and correct synchronisation problems.

See link:../sync-inbound-email-accounts[Sync Inbound Email Accounts] for details.
