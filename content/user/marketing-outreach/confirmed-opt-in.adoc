...
weight: 55
aliases:
  - /user/modules/Confirmed-Opt-In-Settings
  - /user/modules/confirmed-opt-in-settings
---
...
The *Confirmed Opt In* feature provides two opt in settings for email addresses: *Opt In* and *Confirmed Opt In*.
...
the link:../email-templates[Email Templates] module, but note that it must contain the Opt In link.
...
Confirmed Opt In also automatically updates link:../lawful-basis[Lawful Basis] fields on records when the opt-in is confirmed.
