...
The modal is typically opened from a record-level async validator (see link:../../../../../metadata/record-view-metadata/record-async-validators[Record-Level Async Validators]).
...
By default, `relate` type fields in the table have their links disabled to prevent navigation away from the modal. To explicitly control link behaviour per column:

* `['name' => 'name']` — keeps the default link behaviour (name fields open in a new tab)
* `['name' => 'email1', 'link' => false]` — disables the link
* `['name' => 'account_name', 'link' => true]` — forces the link on (opens in a new tab)
...
* **Proceed** — closes the modal and continues the action
...
When `asyncProcess` is set to `true`, the action is dispatched to the backend as an async process. The action key is submitted along with all record IDs currently in the list.

The backend response can include control flags to manage the modal:
