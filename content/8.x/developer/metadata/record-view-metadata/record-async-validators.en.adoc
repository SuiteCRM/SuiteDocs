...
class DuplicateEmailValidationHandler extends LegacyHandler implements ProcessHandlerInterface
{
    ...
    public function __construct(
        ...
        parent::__construct(
            ...
            $defaultSessionName, $legacyScopeState, $requestStack
        );
    }
...
See the link:../../../extensions/frontend/containers/confirmation-modal/confirmation-list-modal[Confirmation List Modal] page for full details on column configuration, preset data handlers, and action buttons.
...
│       └── Contacts/
│           └── metadata/
│               └── detailviewdefs.php                 # Viewdef configuration
└── include/
    └── language/
        └── en_us.lang.php                             # Language labels (or custom/)
----

=== 7.3 Viewdef Configuration

Add the `asyncValidators` entry inside the `DetailView` key of the Contacts `detailviewdefs.php`.

If you do not already have a custom `detailviewdefs.php`, copy the core one:

. Copy `public/legacy/modules/Contacts/metadata/detailviewdefs.php` to `public/legacy/custom/modules/Contacts/metadata/detailviewdefs.php`
. Add the `asyncValidators` key inside the `DetailView` array
...
$viewdefs['Contacts'] = array(
    'DetailView' => array(
        'templateMeta' => array( /* ... existing config ... */ ),
        'panels' => array( /* ... existing panels ... */ ),
...
Alternatively, you can use a `ViewConfigMapper` to add the validator programmatically without modifying the metadata file — see link:./#_2_1_alternative_using_a_viewconfigmapper[Section 2.1].
...
Add the language labels used by the modal. There are two ways to add labels: via the legacy language extension directory, or via a YAML file in your extension.

==== Legacy Language Extension

Add to the Contacts module language file (e.g. `public/legacy/custom/Extension/modules/Contacts/Ext/Language/en_us.duplicate_email.php`):

[source,php]
----
<?php

$mod_strings['LBL_DUPLICATE_EMAIL_TITLE'] = 'Duplicate Email Found';
$mod_strings['LBL_DUPLICATE_EMAIL_MESSAGE'] = 'One or more contacts with the same email address already exist. Do you want to continue?';
$mod_strings['LBL_DUPLICATE_EMAIL_FOUND'] = 'Duplicate email address found';
----

==== Extension Language YAML

Alternatively, add the labels via a YAML file in your extension (e.g. `extensions/defaultExt/modules/Contacts/language/duplicate-email-validation.yaml`).

To add module-level labels (`mod_strings`), use the `module` key with the module name:
...
parameters:
  language:
    en_us:
      module:
        Contacts:
          LBL_DUPLICATE_EMAIL_TITLE: 'Duplicate Email Found'
          LBL_DUPLICATE_EMAIL_MESSAGE: 'One or more contacts with the same email address already exist. Do you want to continue?'
          LBL_DUPLICATE_EMAIL_FOUND: 'Duplicate email address found'
----

To add global app strings instead, use the `application` key:
...
The YAML language files support three key types: `application` (app strings), `module.<ModuleName>` (module strings), and `lists` (dropdown options).
...
. Run `php bin/console cache:clear`
. Run **Repair and Rebuild** from the Admin menu (to pick up the viewdef changes)
...
* link:../../../extensions/frontend/containers/confirmation-modal/confirmation-list-modal[Confirmation List Modal] — the modal variant that displays a searchable table of records
